...
from utils.utils import coletar_endereco_interativo, cartorio
...
def criar_qualificacao_viuvo(dados, caminho):
    ...
    # Estado Civil
    run = p.add_run(f"{dados["viuv"]}")
    run.bold = True

    # Certidão de Casamento com averbação
    p.add_run(f", conforme ")
    run = p.add_run(f"certidão de {dados['certidao']}")
    run.underline = True
...
def pergunta_dados_viuvo():
    ...
    while True:
        certidao = input("Certidão de Casamento ou Óbito (C/O): ").lower()
        if certidao == "c":
            dados["certidao"] = "certidão de casamento com averbação do óbito"
            break
        elif certidao == "o":
            dados["certidao"] = "certidão de óbito"
            break
        else:
            print("Resposta inválida. Escolha (C/O)")

    dados["termo_certidao"] = input("Termo da Certidão: ")
    dados["folhas"] = input("fls.: ")
    dados["livro"] = ("B-" if certidao == "c" else "C-") + input("Livro: ")
