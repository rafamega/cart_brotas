...
def criar_qualificacao_divorciado(dados, caminho):
    doc = Document()
    ...
    # Estado Civil
    run = p.add_run(f"{dados["divorciad"]}")
    run.bold = True

    # Certidão de Casamento com averbação
    p.add_run(f", conforme ")
    run = p.add_run("certidão de casamento com averbação do divórcio")
    run.underline = True
